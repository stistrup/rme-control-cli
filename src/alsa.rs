...
/// Find the RME Babyface Pro card number by USB ID
pub fn find_babyface_card() -> Result<String, String> {
    ...
    for (i, line) in cards.lines().enumerate() {
        // Only read first of the 2 lines per card
        if i % 2 == 0 {
            if let Some(card_index) = line.split_whitespace().next() {
                ...
                if let Ok(usb_id) = fs::read_to_string(&usb_id_path) {
                    // RME Babyface Pro (class compliant mode)
...
/// Get the raw volume value for a control
...
/// Set the raw volume value for a control
...
/// Parse the raw value from amixer output
...
/// Get the state of a boolean switch control (e.g. phantom power, PAD)
...
/// Set the state of a boolean switch control
...
/// Get the current value of an enum control (e.g. line sensitivity)
...
/// Set an enum control value (e.g. line sensitivity)
